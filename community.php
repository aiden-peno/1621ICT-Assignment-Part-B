    ...
    if ($_FILES["community-form-image"]["name"] !== "") {
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($_FILES["community-form-image"]["tmp_name"], $targetFile)) {
                ...
                $successMessage = "Uploaded successfully. Thank you for your submission!";
            } else {
...
if (count($communityPosts) == 0) {
    $searchFormClass = "display-block";
} else {
    $searchFormClass = "display-none";
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div id='main'>
        ...
        <section>
            ...
            <form id="community-search-form" action="community.php" method="GET" class="<?=$searchFormClass?>">
                <label for="community-search-term">Search Text</label>
                <input type="text" id="community-search-term" name="community-search-term" value="<?=$searchTerm?>" />
                <label for="community-search-start-date">Limit Results From</label>
                <input type="date" id="community-search-start-date" name="community-search-start-date" value="<?=$searchDate?>" />
                <input type="submit" value="Submit" />
                    ...
                    <a class="community-post-link" href="<?= $post["IMAGE_LOCATION"] ?>" target="_blank">
                        <section class="community-post">
                            ...
                            <div class="community-post-text">
                                <h3 class="community-post-title"><?= $post["TITLE"] ?>  (<?= $dateString ?>)</h3>
